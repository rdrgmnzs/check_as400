...
--------------------------------------------
Last Modified  2013/07/05 by Shao-Pin, Cheng
--------------------------------------------*/
...
//	System.out.println("      QCMN              = Check QCMN JOB transactions time out.(Only for CUB)");
		...
		try{
			while(flag!=READY_FLAG){
				if(args[i].equals("-H")){
					...
					System.out.println("Modified by Shao-Pin Cheng <cjt74392@ms10.hinet.net>");
					System.out.println("");
					System.exit(0);
				}
				else if(args[i].equals("-v")){
	...
	public static int getStatus(double val){
		...
		if(ARGS.checkVariable==CPU || ARGS.checkVariable==DB || ARGS.checkVariable==JOBS || ARGS.checkVariable==AJOBS || ARGS.checkVariable==OUTQ || ARGS.checkVariable==DBFault || ARGS.checkVariable==QCMN || ARGS.checkVariable==ASP || ARGS.checkVariable==CPUC || ARGS.checkVariable==MIMIX || ARGS.checkVariable==JOBQ ){
	...
	final static int WRKSYSSTS=0,WRKOUTQ=1,DSPMSG=2,DSPSBSD=3,DSPJOB=4,WRKACTJOB=5,CMDLOGIN=6,CMDCLP=7,WRKDSKSTS=8,WRKASPBRM=9,WRKSYSACT=10,DSPDGSTS=11,WRKJOBQ=12,CHKJOBSTS=13;
	//These constants are for the variables
	final static int CPU=0,DB=1,US=2,JOBS=3,MSG=4,OUTQ=5,SBS=6,DJOB=7,AJOBS=8,DBFault=9,QCMN=10,DISK=11,ASP=12,CPUC=13,MIMIX=14,JOBQ=15,JOBSTS=16;
